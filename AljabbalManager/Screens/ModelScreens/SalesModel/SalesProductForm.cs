...
{
    public partial class SalesProductForm : TemplateForm
    {
        SaleInvoiceForm _ParentForm;
        string _ProductCode = null;
        public SalesProductForm(SaleInvoiceForm obj)
        {
            InitializeComponent();
            _ParentForm = obj;
        }
        ...
        private bool ProductQuantityExist()
        {
            int PQ = 0;
            try
            {
                ...
                string conString = AppSetting.ConnectionString();
                string _Command = @"SELECT        ProductStock.ProductQuantity
FROM            ProductsDetail INNER JOIN
                         ProductStock ON ProductsDetail.ProductCode = ProductStock.ProductCode
WHERE        (ProductsDetail.ProductName =@ProductName ) AND (ProductsDetail.ProductDescription =@ProductDescription )";
                using (SqlConnection conn = new SqlConnection(conString))
                {
                    using (SqlCommand cmd = new SqlCommand(_Command, conn))
                    {
                        ...
                        reader.Read();
                        PQ = Convert.ToInt32(reader["ProductQuantity"].ToString());
                    }
                }
        ...
        private bool GetDataFromProductDetail()
        {
            try
            {
                string conString = AppSetting.ConnectionString();
                ...
                using (SqlConnection conn = new SqlConnection(conString))
                {
                    using (SqlCommand cmd = new SqlCommand(_Command, conn))
                    {
                        ...
                        SqlDataReader reader = cmd.ExecuteReader();
                        reader.Read();
                        _ProductCode = reader["ProductCode"].ToString();
                        _ParentForm.GetProductDetail(reader["ProductCode"].ToString(), reader["ProductDescription"].ToString(), reader["ProductUnit"].ToString());
                        this.Dispose();
                    }
                }
            }
        ...
        private void GetDataFromProductStock()
        {
            ...
            {
                string conString = AppSetting.ConnectionString();
                string _Command = @"SELECT        ProductRetailPrice, ProductQuantity
                                    FROM            ProductStock
                ...
                using (SqlConnection conn = new SqlConnection(conString))
                {
                    using (SqlCommand cmd = new SqlCommand(_Command, conn))
                    {
                        ...
                        SqlDataReader reader = cmd.ExecuteReader();
                        reader.Read();
                        _ParentForm.GetProductStock(reader["ProductRetailPrice"].ToString(), reader["ProductQuantity"].ToString()); 
                    }
                }
