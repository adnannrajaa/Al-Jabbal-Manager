...
{
    public partial class SelectProductForm : TemplateForm
    {
        StockDetailForm __StockDetailForm;
        public SelectProductForm(StockDetailForm obj)
        {
            InitializeComponent();
            __StockDetailForm = obj;
        }
        ...
        private void OkButton_Click(object sender, EventArgs e)
        {
            if (IsFormValid())
            {
                ...
                {
                    string conString = AppSetting.ConnectionString();
                    string _Command = @"Select ProductCode From ProductsDetail where ProductName=@ProductName and ProductDescription=@ProductDescription";
                    using (SqlConnection conn = new SqlConnection(conString))
                    {
                        using (SqlCommand cmd = new SqlCommand(_Command, conn))
                        {
                            ...
                            reader.Read();
                            __StockDetailForm.GetProductCode(reader["ProductCode"].ToString());
                            this.Dispose();
